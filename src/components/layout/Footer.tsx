...
const socialLinks = [
  {
    name: 'LinkedIn',
    href: 'https://www.linkedin.com/company/prepoc-technologies/posts/?feedView=all',
    icon: (
  ...
  {
    name: 'Facebook',
    href: 'https://www.facebook.com/profile.php?id=61570804714478',
    icon: (
      <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
  ...
  {
    name: 'Instagram',
    href: 'https://www.instagram.com/pre.poc/',
    icon: (
      <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
...
export default function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Handle non-JSON error pages (like Next.js 500 dev overlays) gracefully
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row w-full md:w-auto gap-3">
              <div className="relative flex-1 md:w-64">
                <input
                  ...
                  className={`w-full bg-white/5 border ${error ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-primary/50'} rounded-full px-5 py-3 text-sm text-foreground placeholder:text-muted focus:outline-none transition-colors disabled:opacity-50`}
                  aria-label="Email address for newsletter"
                />
                {error && (
                  <p className="absolute -bottom-5 left-4 text-[10px] text-red-400">{error}</p>
                )}
              </div>
              <button
                type="submit"
                disabled={isSubmitting || isSuccess}
                className="btn-primary text-sm py-3 px-5 flex-shrink-0 w-full sm:w-auto disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
